...
class ResNetCifar(nn.Module):
    def __init__(self, depth, shape, freeze=False, shared=None, width=1, classes=10, channels=3, norm_layer=nn.BatchNorm2d):
    ...
    def forward(self, x):
        ...
        if self.shared == 'layer2':
            ...
            z, log_det_J = self.uns(a)
        x = self.layer3(x)
        if self.shared == 'layer3' or self.shared is None:
            a = torch.flatten(x, start_dim=1)
            z, log_det_J = self.uns(a)
        x = self.bn(x)
        x = self.relu(x)
